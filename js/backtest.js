// js/backtest.js – Backtest usando o MESMO motor do live (corrigido)
...
// Busca o elemento mais recente que atende à condição (varre de trás para frente)
...
export async function runBacktest(symbol = 'BTCUSDT', days = 30) {
    ...
    try {
        ...
        // Macro estático (Alpha Vantage) + Fear & Greed (estático, mas pode ser melhorado)
        const macroData = await fetchMacroStatic();

        // 3. Estado do ativo (espelhando assetsData do live)
        const state = {
            candles1H: [],
            candles4H: candles4h || [],
            ema50_1H: 0,
        ...
        function updateIndicators(candles) {
            ...
            // ADX (pode retornar objeto com adx, plusDI, minusDI)
            const adxData = calculateADX(candles);
            state.adx = adxData; // guarda o objeto completo
            ...
            // Divergência RSI (simplificada, mas mantendo compatibilidade)
            let rsiValues = [];
            if (closes.length > 20) {
                // Recalcular RSI para divergência (usar função existente ou simplificar)
                // Vamos deixar null para não complicar, mas o motor principal pode usar.
                // Para simular, usamos a função detectRSIDivergence com dados básicos
                const rsiForDiv = [];
                let g = 0,
                    l = 0;
                for (let i = 1; i < closes.length; i++) {
                    ...
                    if (i <= 14) {
                        ...
                        const gain = d > 0 ? d : 0;
                        const loss = d < 0 ? -d : 0;
                        let ag = (rsiForDiv[rsiForDiv.length - 1] * 13 + gain) / 14; // simplificado
                        let al = (rsiForDiv[rsiForDiv.length - 1] * 13 + loss) / 14;
                        rsiForDiv.push(al === 0 ? 100 : 100 - (100 / (1 + ag / al)));
                    }
                }
        ...
        for (let i = 0; i < filteredCandles.length; i++) {
            ...
            // Macro blackout (usando o mesmo do live, com fallback)
            const macroCheck = await isHighImpactEventNow();
            state.macroBlackout = macroCheck.isBlackout && macroCheck.impact === 'HIGH';

            // Fear & Greed (estático para backtest, mas podemos buscar histórico se quiser)
            // Para simplificar, usamos 50 (neutro)
            ...
            if (position) {
                ...
                if (closed) {
                    ...
                    // Atualizar trailing com base nos swings
                    if (position.type === 'LONG') {
            ...
            if (!position && !blockReason && primaryDirection) {
                ...
                if (smcSetup && retestConfirmed) {
                    ...
                    if (rr1 < 1.5) continue; // (vai para o próximo candle)
                    ...
                    // Aplicar multiplicador do Fear & Greed (estático)
                    const fgMultiplier = 1; // no live usa fearGreedFilter
                    const riskPct = Math.min(kellyPct * fgMultiplier, 0.05); // máximo 5%
                    ...
                    position = {
                        ...
                        trailingStop: stop,
                        partialTaken: false,
                        sizeRemaining: riskPct, // <-- aplica o sizing fracionado
        ...
        // Fechar posição remanescente (último candle)
        if (position) {
